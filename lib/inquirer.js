...
const createCards = (employees) => {
    return new Promise(function(resolve){
        for(const el of employees){
            ...
            // injectCode(cardHTML,`{{name}}`, el.getName())
            // .then((source) => injectCode(source,`{{title}}`, el.getRole()))
            // .then((source) => injectCode(source,`{{id}}`, el.getId()))
            // .then((source) => injectCode(source,`{{email}}`, `<a href="mailto:${el.getEmail()}">${el.getEmail()}</a>`))
            // .then((source) => {
            //     switch (el.title){
            //     case 'manager':
            //         injectCode(source,`{{varies}}`, `Office Number: ${el.getOfficeNumber()}`);
            //         break
            //     case 'engineer':
            //         injectCode(source,`{{varies}}`, `GitHub: <a href="${el.getGithub()}" target="_blank">${el.getGithub()}</a>`);
            //         break
            //     case 'intern':
            //         injectCode(source,`{{varies}}`, `School: ${el.getSchool()}`);
            //         break
            //     default:
            //         injectCode(source,`{{varies}}`, "");
            //         break
            //     };
            // })
            // .then((source) => {
...
//append cards into html template file
